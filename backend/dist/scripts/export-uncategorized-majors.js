...
async function main() {
    ...
    await ds.initialize();
    const rows = await ds.query('SELECT id, name, code, field_group FROM majors ORDER BY name');
    const uncategorized = rows
        .map((r) => ({
        ...r,
        canonical_group: (0, major_normalization_1.classifyMajor)(r.name, r.field_group).primary_group_name,
    }))
        .filter((r) => r.canonical_group === 'Khác');
    const rawOnly = uncategorized.filter((r) => !r.field_group?.trim() ||
    ...
    const csvLines = [
        ...
        ...uncategorized.map((r) => `${r.id},"${r.name.replace(/"/g, '""')}","${r.code ?? ''}","${(r.field_group ?? '').replace(/"/g, '""')}",${r.canonical_group}`),
    ];
...
//# sourceMappingURL=export-uncategorized-majors.js.map