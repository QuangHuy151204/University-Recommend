...
exports.API_SPOT_CHECK_IDS = void 0;
exports.resolveMajorIdFromCatalog = resolveMajorIdFromCatalog;
exports.buildUniversityFilterScenarios = buildUniversityFilterScenarios;
const major_normalization_1 = require("../src/majors/major-normalization");
exports.API_SPOT_CHECK_IDS = [
    'combo-a01',
    'score-27',
    'major-cntt',
    'major-httt',
...
function resolveMajorIdFromCatalog(catalog, name) {
    const exact = catalog.find((m) => m.name === name);
    if (exact)
        return exact.id;
    const canon = (0, major_normalization_1.canonicalMajorName)(name);
    const byCanon = catalog.find((m) => (0, major_normalization_1.canonicalMajorName)(m.name) === canon);
    if (byCanon)
        return byCanon.id;
    const partial = catalog.find((m) => (0, major_normalization_1.canonicalMajorName)(m.name).includes(canon) ||
        canon.includes((0, major_normalization_1.canonicalMajorName)(m.name)));
    return partial?.id;
}
function buildUniversityFilterScenarios(catalog) {
    const m = (name) => resolveMajorIdFromCatalog(catalog, name);
    const withMajor = (id, label, query) => {
        const { major, ...rest } = query;
        if (!major)
            return { id, label, query: rest };
...
//# sourceMappingURL=university-filter-scenarios.js.map