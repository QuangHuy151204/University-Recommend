...
exports.classifyMajor = classifyMajor;
exports.majorBelongsToGroup = majorBelongsToGroup;
...
function patternMatches(normalizedSource, pattern, match = 'contains') {
    if (match === 'exact')
        return normalizedSource === pattern;
    if (match === 'startsWith')
        return normalizedSource.startsWith(pattern);
    const escaped = pattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    if (pattern.length <= 3) {
        return new RegExp(`(?:^|\\s)${escaped}(?:\\s|$)`, 'u').test(normalizedSource);
    }
...
function groupNameForId(groupId) {
    ...
    return major_groups_catalog_1.MAJOR_GROUP_BY_ID.get(groupId)?.group_name ?? major_groups_catalog_1.KHAC_GROUP_NAME;
}
...
function classifyMajor(majorName, rawFieldGroup) {
    const normalizedName = normalizeText(majorName);
    const orderedIds = [];
    const tags = [];
    const addGroup = (groupId) => {
        if (!orderedIds.includes(groupId))
            orderedIds.push(groupId);
    };
    for (const rule of DISAMBIGUATION_RULES) {
        const matched = rule.patterns.some((p) => patternMatches(normalizedName, p, rule.match ?? 'contains'));
        if (!matched)
            continue;
    ...
    for (const rule of DOMAIN_RULES) {
        const matched = rule.patterns.some((p) => patternMatches(normalizedName, p, 'contains'));
        if (matched) {
    ...
    if (orderedIds.length === 0) {
        orderedIds.push(major_groups_catalog_1.KHAC_GROUP_ID);
    }
    return finalizeClassification(orderedIds, tags, majorName);
}
function finalizeClassification(orderedIds, ruleTags, majorName) {
    const nameDerived = nameTokens(majorName).slice(0, 8);
    const allTags = dedupeTags([...ruleTags, ...nameDerived]);
    const primaryId = orderedIds[0] ?? major_groups_catalog_1.KHAC_GROUP_ID;
...
function majorBelongsToGroup(majorName, rawFieldGroup, groupIdOrSlug, storedGroupIds) {
...
//# sourceMappingURL=major-classification.js.map