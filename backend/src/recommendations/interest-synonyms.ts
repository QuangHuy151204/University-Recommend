...
function normalizeInterestKey(input: string): string {
  return input
    .toLowerCase()
    .normalize('NFD')
    .replace(/\p{M}/gu, '')
...
/** Mở rộng danh sách cụm sở thích (gồm đồng nghĩa) để khớp ngành. */
export function expandInterestPhrases(phrases: string[]): string[] {
